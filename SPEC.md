...
- 2026-07-18 · M6 Packaging e delivery: `deploy/install.sh` (installer Ubuntu 24 — prerequisiti+Docker, rilevamento GPU con fallback CPU, wizard `.env` minimo, `compose up`, pull modello, healthcheck finale; flag `--non-interactive`), interruttore LLM **bundled/external via `COMPOSE_FILE`** (`docker-compose.llm.yml` + override `docker-compose.gpu.yml`; stack dev intatto), `deploy/backup.sh` (pg_dump + snapshot Qdrant via API + repository + `.env`, page cache esclusa) e `deploy/restore.sh` (distruttivo, non tocca il `.env` live) con round-trip verificato da `deploy/test-backup-restore.sh`; Caddy TLS self-signed di default (cert cliente documentato); guida `docs/install/guida-installazione.md` per consulente IT; target `make install|backup|restore|test-restore`. `make lint`, `make test` (143 test) e `make licenses` verdi. Decisioni in `docs/adr/0007`.
- 2026-07-18 · M6 DoD verificata su host reale (Docker Desktop, profilo GPU). Tre fix emersi dalla verifica end-to-end: (1) tag modello di default `qwen3.5:9b-instruct-q4_K_M` era **404 sul registry Ollama** → corretto in `qwen3.5:9b-q4_K_M` (`.env.example`, `config.py`, guida, spec) prima che `install.sh` fallisse al pull; (2) `restore.sh` eseguiva la recovery dello snapshot Qdrant tramite il container `api` **dopo averlo fermato** → ora gira da un container `api` effimero (`compose run --rm --no-deps`); (3) `restore.sh` cancellava `data/repository` da host con `rm -rf`, ma i file sono scritti dal container root → ora wipe+extract del repository avviene dentro un container root (stdlib Python). `install.sh` → stack sano + login admin; corpus di 7 PDF indicizzato (incl. OCR) con chat citata dal modello bundled su GPU; `test-backup-restore.sh` → **ROUND-TRIP PASSED**. M0→M6 completa.
- 2026-07-18 · Retrieval quality (post-M6, emerso da test con documenti reali): (1) **FTS ora
  language-agnostic** — il `tsv` era indicizzato con la config lingua del *documento* e la query
  con quella della *domanda*; quando divergono (fattura IT rilevata `en`, query di sole
  parole-chiave) lo stemming non combaciava e la ricerca lessicale falliva in silenzio.
  `CHUNK_TSV_EXPRESSION` e la query passano entrambi a `to_tsvector('simple', …)`; migrazione
  `0003` che ricostruisce il `tsv` con DROP+ADD della colonna generata (Postgres 16 non ha
  `SET EXPRESSION`, che è PG17+). Nessun re-embed. (2) **Espansione ai chunk vicini** in
